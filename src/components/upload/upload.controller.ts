...
export default class UploadController {
	public static async uploadAcademicAdministrationEvidenceAWS(file: any, userId: string, activityId: string) {
	...
	public static async uploadResearchEvidenceAWS(file: any, userId: string, activityId: string) {
		const { createReadStream, filename, mimetype } = await file;
		const stream = createReadStream();
		...
		return new Promise((resolve, reject) => {
			return s3.upload(params, async (err: any, data: any) => {
				...
				if (data) {
					...
					const update = await UserController.assignProfilePicture(userId, data.Location);
